...
function urlResolve (vk, url) {
	...
	} else if (tmp = url.match(/vk.com\/(public|event)([a-zA-Z\_]+)/)) {
...
(new Slave ({
	...
	.use ('urn:fos:sync:feature/e242b98044c627d2009df1ad92775771', function reply (task) {
		...
		if (tmp = task.url.match (/\/wall(|\-)(\d+)\_(\d+)/)) {
			var owner_id = tmp [1] + tmp [2],
				post_id = tmp [3],
				type = 'wall_post';
		} else if (tmp = task.url.match (/\/topic(|\-)(\d+)\_(\d+)\?reply=(\d+)/)) {
			var group_id = tmp [2],
				topic_id = tmp [3],
				reply_id = tmp [4],
				type = 'topic_post';
			...
			var group_id = tmp [2],
				topic_id = tmp [3],
				type = 'topic_post';
		} else if (tmp = task.url.match (/\/id(\d+)/)) {
			var user_id = tmp [1],
				type = 'message';
		}
	...
	.use ('urn:fos:sync:feature/1f1d48152476612c3d5931cb9239fc2a', function searchAllNews (task) {
		...
		return iterate (vk, 'newsfeed.search', {q: keywords, count: 10}, filter, callback);
	})
